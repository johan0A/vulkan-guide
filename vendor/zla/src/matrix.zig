...
pub fn Mat(comptime T: type, comptime cols_: usize, comptime rows_: usize) type {
    return extern struct {
        ...
        /// create a perspective projection matrix (TODO: Vulkan clip space [0,1])
        ...
        /// Reverse-Z perspective projection (TODO: Vulkan clip space [0,1], infinite far plane)
        pub fn perspectiveReverseZ(fovy: T, aspect: T, near: T) Self {
            if (rows != 4 or cols != 4) @compileError("Perspective matrix must be 4x4");

            const f = 1.0 / std.math.tan(fovy / 2.0);

            var result: Self = .zero;
            result.items[0][0] = f / aspect;
            result.items[1][1] = f;
            result.items[2][3] = -1.0;
